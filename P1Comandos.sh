 1. Listar todos los archivos del directorio bin.

[ferhandel@webminal.org ~]$ls /bin
ls: cannot access /bin/kdumpctl: Permission denied
ls: cannot access /bin/systemd-hwdb: Permission denied
ls: cannot access /bin/systemd-tmpfiles: Permission denied
ls: cannot access /bin/fusermount: Permission denied
ls: cannot access /bin/numad: Permission denied
ls: cannot access /bin/cloud-init: Permission denied
ls: cannot access /bin/mysql_upgrade: Permission denied
ls: cannot access /bin/mandb: Permission denied
ls: cannot access /bin/udevadm: Permission denied
ls: cannot access /bin/mysqld_safe: Permission denied
ls: cannot access /bin/systemctl: Permission denied
ls: cannot access /bin/systemd-tty-ask-password-agent: Permi
ssion denied
ls: cannot access /bin/systemd-notify: Permission denied
ls: cannot access /bin/redis-server: Permission denied
[
2to3
a2p
ac
aclocal
aclocal-1.13
addr2line
alias
amuFormat.sh
animalsay
animate
apropos
ar
arch
aria_chk
aria_dump_log
aria_ftdump
aria_pack
aria_read_log
as
asadmin
aserver
attr
audit2allow
audit2why
aulast
aulastlog
ausyscall
autoconf
autoheader
autom4te
automake
automake-1.13
autoreconf
autoscan
autoupdate
auvirt
awk
basename
bash
bashbug
bashbug-64
bdftopcf
bdftruncate
bg
bond2team
bootctl
bundle_image
bunzip2
busctl
bzcat
bzcmp
bzdiff
bzgrep
bzip2
bzip2recover
bzless
bzmore
c2ph
c89
c99
cairo-sphinx
cal
ca-legacy
captoinfo
cat
catchsegv
catman
cc
cd
celtdec051
celtenc051
centrino-decode
certutil
cfadmin
c++filt
chacl
chage
chardetect
chattr
chcat
chcon
checkmodule
checkpolicy
cheetah
cheetah-analyze
cheetah-compile
chfn
chgrp
chmod
chown
chrt
chsh
chvt
ckbcomp
cksum
clear
cloud-init                                                  
cloud-init-per
cmp
cmsutil
col
colcrt
colrm
column
comm
command
compare
compile_et
composite
conjure
convert
coredumpctl
cowsay
cowthink
cp
cpio
cpp
cpupower
cq
creategif
crlutil
crontab
csplit
csslint-0.6
curl
cut
cwutil
date
db_archive
db_checkpoint
db_deadlock
db_dump
db_dump185
db_hotbackup
dbilogstrip
dbiprof
dbiproxy
db_load
db_log_verify
db_printlog
db_recover
db_replicate
db_stat
db_tuner
db_upgrade
dbus-binding-tool
dbus-cleanup-sockets
dbus-daemon
dbus-monitor
dbus-run-session
dbus-send
dbus-test-tool
dbus-update-activation-environment
dbus-uuidgen
db_verify
dconf
dd
deallocvt
debuginfo-install
desktop-file-edit
desktop-file-install
desktop-file-validate
df
dgawk
dialog
diff
diff3
dir
dircolors
dirname
display
dmesg
dnsdomainname
domainname
dotlockfile
dracut
du
dumpkeys
dvipdf
dwp
dwz
dynamodb_dump
dynamodb_load
easy_install
easy_install-2.7
easy_install-3.5
ebrowse
echo
egrep
eject
elbadmin
elfedit
emacs
emacs-24.3
emacsclient
env
envsubst
eps2eps
eqn
erb
etags
etags.emacs
ex
expand
expr
factor
fallocate
false
fc
fc-cache
fc-cat
fc-list
fc-match
fc-pattern
fc-query
fc-scan
fc-validate
fetch_file
fg
fgconsole
fgrep
file
find
find2perl
findcon
findmnt
find-repos-of-install
fipscheck
fipshmac
firewall-cmd
firewall-offline-cmd
flock
fmt
fold
font2c
fonttosfnt
free
funzip
fusermount                                                  
gapplication
gawk
gcc                                                        
gcc-ar
gcc-nm
gcc-ranlib
gconf-merge-tree
gconftool-2
gcore
gcov
gctags
gdb
gdb-add-index
gdbus
gdk-pixbuf-query-loaders-64
gem
gencat
genisoimage
genl-ctrl-list
geoiplookup
geoiplookup6
geoipupdate
geqn
getconf
getent
getfacl
getfattr
gethostip
getkeycodes
getopt
getopts
gettext
gettext.sh
ghostscript
gio
gio-querymodules-64
glacier
glib-compile-schemas
gmake
gneqn
gnroff
gpasswd
gpg
gpg2
gpg-agent
gpgconf
gpg-connect-agent
gpg-error
gpgparsemail
gpgsplit
gpgv
gpgv2
gpg-zip
gpic
gprof
gr2fonttest
grep
grep-changelog
groff
grops
grotty
groups
growpart
grub2-editenv
grub2-file
grub2-fstest
grub2-glue-efi
grub2-kbdcomp
grub2-menulst2cfg
grub2-mkfont
grub2-mkimage
grub2-mklayout
grub2-mknetdir
grub2-mkpasswd-pbkdf2
grub2-mkrelpath
grub2-mkrescue
grub2-mkstandalone
grub2-render-label
grub2-script-check
grub2-syslinux2cfg
gs
gsbj
gsdj
gsdj500
gsettings
gsettings-data-convert
gslj
gslp
gsnd
gsoelim
gss-client
gstack
gtar
gtbl
gtk-launch
gtk-query-immodules-2.0-64
gtk-query-immodules-3.0-64
gtk-update-icon-cache
gtroff
guestfish
guestmount
guestunmount
gunicorn
gunicorn_paster
gunzip
gzexe
gzip
h2ph
hdsploader
head
hexdump
hexedit
hivexget
hivexml
hivexregedit
hivexsh
hostid
hostname
hostnamectl
i386
iconv
id
identify
idiag-socket-details
idle
idn
ifnames
igawk
import
info
infocmp
infokey
infotocap
innochecksum
install
instance_events
ionice
ipcalc
ipcmk
ipcrm
ipcs
iptables-xml
irb
isodebug
isodump
isohybrid
isoinfo
isosize
isovfy
jemalloc.sh
jobs
join
journalctl
jsonpointer
json_reformat
json_verify
kbdinfo
kbd_mode
kbdrate
kdumpctl                                                    
kernel-install
keyctl
kill
killall
kill_instance
kmod
krb5-config
last
lastb
lastcomm
lastlog
launch_instance
lchfn
lchsh
ld
ld.bfd
ldd
ld.gold
less
lessecho
lesskey
lesspipe.sh
lexgrog
libguestfs-test-tool
libtool
libtoolize
link
linux32
linux64
linux-boot-prober
list_instances
ln
loadkeys
loadunimap
locale
localectl
localedef
logger
login
loginctl
logname
look
lprsetup.sh
ls
lsattr
lsblk
lscpu
lsinitrd
lsipc
lslocks
lslogins
lsns
lss3
lsscsi
lua
luac
lz
lz4
lz4c
lz4cat
lzop
m17n-conv
m4
machinectl
mailq
mailq.postfix
make
makedb
man
mandb                                                      
manpath
mapscrn
markdown_py
markdown_py-2.7
mattrib
mbadblocks
mc
mcat
mcd
mcdiff
mcedit
mcheck
mclasserase
mcomp
mcookie
mcopy
mcview
md5sum
mdel
mdeltree
mdir
mdu
memdiskfind
mesg
mformat
minfo
mixartloader
mkdir
mkfifo
mkfontdir
mkfontscale
mkhybrid
mkinitrd
mkisofs
mkmanifest
mknod
mktemp
mlabel
mmd
mmount
mmove
modutil
mogrify
montage
more
mount
mountpoint
mpartition
mrd
mren
msgattrib
msgcat
msgcmp
msgcomm
msgconv
msgen
msgexec
msgfilter
msgfmt
msgfmt.py
msggrep
msghack
msginit
msgmerge
msgunfmt
msguniq
mshortname
mshowfat
msql2mysql
mtools
mtoolstest
mturk
mtype
mv
mxtar
myisamchk
myisam_ftdump
myisamlog
myisampack
my_print_defaults
mysql
mysqlaccess
mysqladmin
mysqlbinlog
mysqlbug
mysqlcheck
mysql_convert_table_format
mysqld_multi
mysqld_safe                                                
mysqldump
mysqldumpslow
mysql_find_rows
mysql_fix_extensions
mysqlhotcopy
mysqlimport
mysql_install_db
mysql_plugin
mysql_secure_installation
mysql_setpermission
mysqlshow
mysqlslap
mysqltest
mysql_tzinfo_to_sql
mysql_upgrade                                              
mysql_waitpid
mysql_zap
mzip
namei
nano
nc
ncat
ndptool
needs-restarting
neqn
netstat
nettle-hash
nettle-lfib-stream
newaliases
newaliases.postfix
newgrp
nf-ct-add
nf-ct-list
nf-exp-add
nf-exp-delete
nf-exp-list
nf-log
nf-monitor
nf-queue
ngettext
nginx-upgrade
nice
nisdomainname
nl
nl-addr-add
nl-addr-delete
nl-addr-list
nl-class-add
nl-class-delete
nl-classid-lookup
nl-class-list
nl-cls-add
nl-cls-delete
nl-cls-list
nl-fib-lookup
nl-link-enslave
nl-link-ifindex2name
nl-link-list
nl-link-name2ifindex
nl-link-release
nl-link-set
nl-link-stats
nl-list-caches
nl-list-sockets
nl-monitor
nl-neigh-add
nl-neigh-delete
nl-neigh-list
nl-neightbl-list
nl-pktloc-lookup
nl-qdisc-add
nl-qdisc-delete
nl-qdisc-list
nl-route-add
nl-route-delete
nl-route-get
nl-route-list
nl-rule-list
nl-tctree-list
nl-util-addr
nm
nmcli
nm-online
nmtui
nmtui-connect
nmtui-edit
nmtui-hostname
nohup
nproc
nroff
nsenter
numad                                                      
numfmt
objcopy
objdump
od
oldfind
open
openssl
openvt
os-prober
otfdump
otflist
otftobdf
otfview
p11-kit
package-cleanup
pango-view
passwd
paste
pathchk
pcre-config
pdf2dsc
pdf2ps
peekfd
perl
perl5.16.3
perlbug
perldoc
perlthanks
perror
pf2afm
pfbtopfa
pgawk
pgrep
pic
piconv
pinentry
pinentry-curses
ping
ping6
pinky
pip
pip2
pip2.7
pip3
pip3.5
pitchplay
pk12util
pkaction
pkcheck
pkcs1-conv
pkexec
pkg-config
pkill
pkla-admin-identities
pkla-check-authorization
pkttyagent
pl2pm
pldd
plymouth
pmap
pod2html
pod2man
pod2text
pod2usage
post-grohtml
powernow-k8-decode
pphs
pr
preconv
pre-grohtml
printafm
printenv
printf
prlimit
prove
prtstat
ps
ps2ascii
ps2epsi
ps2pdf
ps2pdf12
ps2pdf13
ps2pdf14
ps2pdfwr
ps2ps
ps2ps2
psed
psfaddtable
psfgettable
psfstriptable
psfxtable
pstack
pstree
pstree.x11
pstruct
ptx
publishgif
pwd
pwdx
pwmake
pwscore
pyami_sendmail
pydiscoursecli
pydoc
pydoc3.5
pygettext.py
pygmentize
pynche
pyrsa-decrypt
pyrsa-decrypt-2
pyrsa-decrypt-bigfile
pyrsa-decrypt-bigfile-2
pyrsa-encrypt
pyrsa-encrypt-2
pyrsa-encrypt-bigfile
pyrsa-encrypt-bigfile-2
pyrsa-keygen
pyrsa-keygen-2
pyrsa-priv2pub
pyrsa-priv2pub-2
pyrsa-sign
pyrsa-sign-2
pyrsa-verify
pyrsa-verify-2
python
python2
python2.7
python2.7-config
python2-config
python3.5
python3.5m
python-config
pyvenv-3.5
qemu-img
qemu-io
qemu-nbd
quota
quotasync
ranlib
raw
rdoc
read
readelf
readlink
readmult
realpath
recode-sr-latin
redis-benchmark
redis-check-aof
redis-check-rdb
redis-cli
redis-sentinel
redis-server                                                
redis-shutdown
rename
renice
replace
repoclosure
repodiff
repo-graph
repomanage
repoquery
repo-rss
reposync
repotrack
reset
resizecons
resolveip
resolve_stack_dump
rev
ri
rm
rmail
rmail.postfix
rmdir
rnano
route53
rpcgen
rpm
rpm2cpio
rpmdb
rpmkeys
rpmquery
rpmverify
rq
rqinfo
rqworker
rsync
rsyslog-recover-qi.pl
ruby
runcon
run-parts
rvi
rview
rvim
rxe_cfg
s2p
s3put
sandbox
scp
screen
script
scriptreplay
scrub
sdbadmin
sdiff
sechecker
secon
sed
sediff
sedismod
sedispol
seinfo
semodule_deps
semodule_expand
semodule_link
semodule_package
semodule_unpackage
sepolgen
sepolgen-ifgen
sepolgen-ifgen-attr-helper
sepolicy
seq
sesearch
setarch
setfacl
setfattr
setfont
setkeycodes
setleds
setmetamode
setpriv
setsid
setterm
setupcon
setup-nsssysinit
setup-nsssysinit.sh
setvtrgb
sexp-conv
sftp
sg
sh
sha1sum
sha224sum
sha256sum
sha384sum
sha512sum
show-changed-rco
showconsolefont
show-installed
showkey
shred
shuf
siab
signtool
signver
sim_client
size
skill
slabtop
sleep
slogin
smtpd.py
snice
soelim
sort
sotruss
splain
split
sprof
sqlite3
ssh
ssh-add
ssh-agent
ssh-copy-id
ssh-keygen
ssh-keyscan
ssltap
stat
stdbuf
strace
strace-log-merge
stream
strings
strip
stty
su
sudo
sudoedit
sudoreplay
sum
supermin5
sync
syslinux
systemctl                                                  
systemd-analyze
systemd-ask-password
systemd-cat
systemd-cgls
systemd-cgtop
systemd-coredumpctl
systemd-delta
systemd-detect-virt
systemd-escape
systemd-firstboot
systemd-hwdb                                                
systemd-inhibit
systemd-loginctl
systemd-machine-id-setup
systemd-notify                                              
systemd-nspawn
systemd-path
systemd-run
systemd-stdio-bridge
systemd-sysv-convert
systemd-tmpfiles                                            
systemd-tty-ask-password-agent                              
tabs
tac
tail
tailf
tar
taskadmin
taskset
tbl
tclsh
tclsh8.5
teamd
teamdctl
teamnl
tee
termtosvg
test
testgdbm
testrb
tgz
tic
timedatectl
timeout
tload
tmon
toe
top
touch
tput
tr
tracepath
tracepath6
troff
true
truncate
trust
tset
tsort
tty
ttystudio
turbostat
tzselect
ucs2any
udevadm                                                    
ul
ulockmgr_server
umask
umount
unalias
uname
unexpand
unicode_start
unicode_stop
uniq
unison
unison-2.40
unison-gtk-2.40
unix-lpr.sh
unlink
unlz4
unshare
unxz
unzip
unzipsfx
update-ca-trust
update-desktop-database
updated_rootfs.img                                          
update-gtk-immodules
update-mime-database
uploadgif
uptime
urlgrabber
users
usleep
ustr-import
usx2yloader
utmpdump
uuclient
uuidgen
uz
vdir
verifytree
vi
view
vim
vimdiff
vimtutor
virt-alignment-scan
virt-builder
virt-cat
virt-copy-in
virt-copy-out
virt-customize
virt-df
virt-diff
virt-edit
virt-filesystems
virt-format
virt-get-kernel
virt-index-validate
virt-inspector
virt-log
virt-ls
virt-make-fs
virt-rescue
virt-resize
virt-sparsify
virt-sysprep
virt-tail
virt-tar-in
virt-tar-out
virtualenv
virtualenv-2
virtualenv-2.7
virt-win-reg
vlock
vmlinux
vmstat
vxloader
w
wait
wall
watch
watchgnupg
wc
wdctl
wftopfa
wget
whatis
wheel
whereis
which
whiptail
who
whoami
wish
wish8.5
wmsudo
write
x86_64
x86_64-redhat-linux-gcc                                    
x86_energy_perf_policy
xargs
xgettext
xmlcatalog
xmllint
xmlwf
xorg-x11-fonts-update-dirs
xsltproc
xxd
xz
xzcat
xzcmp
xzdec
xzdiff
xzegrep
xzfgrep
xzgrep
xzless
xzmore
yes
ypdomainname
yum
yum-builddep
yum-config-manager
yum-debug-dump
yum-debug-restore
yumdownloader
yum-groups-manager
zcat
zcmp
zdiff
zegrep
zfgrep
zforce
zgrep
zip
zipcloak
zipdetails
zipgrep
zipinfo
zipnote
zipsplit
zless
zmore
znew
zsh
zsoelim
 
2. Listar todos los archivos del directorio tmp.
 
[ferhandel@webminal.org ~]$ls /tmp
npm-26730-c0468a8c
systemd-private-004dcdfb8b534bca8787eeaabb6516a4-mariadb.service-1
ure7N
systemd-private-004dcdfb8b534bca8787eeaabb6516a4-nginx.service-Q4C
Gmn
systemd-private-2c207ee2ffc341c88a3f87baa15dbca0-systemd-hostnamed
.service-sm8cl8
systemd-private-2ce15dda675d49beba2f0ba0be4c20db-mariadb.service-Q
Na71h
systemd-private-2ce15dda675d49beba2f0ba0be4c20db-nginx.service-Jvu
QqN
systemd-private-584845539c4c44cb8dc39ab903487851-mariadb.service-Y
nT5Vz
systemd-private-584845539c4c44cb8dc39ab903487851-nginx.service-4ZD
3ST
systemd-private-604cbf0f4a7141dca9a6e7f2bddb3945-mariadb.service-e
YcOtG
systemd-private-66f8de97ba8f4bc2ac8be511c329ad3d-mariadb.service-u
VFmxF
systemd-private-77239136a389441a82ec536337818e34-mariadb.service-G
usKnX
systemd-private-77239136a389441a82ec536337818e34-nginx.service-W1q
4b2
systemd-private-872ab31ff2c94a53a894dd3f7c9b948a-mariadb.service-s
Pdhsp
systemd-private-872ab31ff2c94a53a894dd3f7c9b948a-nginx.service-PRy
mHl
systemd-private-88c94989f2284b60a23f9872273b2121-mariadb.service-t
SkHeK
systemd-private-8d3d3a6371e54780b2269213ee8559ff-systemd-hostnamed
.service-SPo3GZ
systemd-private-969092f139924bee89708e610a1494ca-systemd-hostnamed
.service-2p7Eem
systemd-private-9ac54a05c02e46f68e1d3c49b100791b-systemd-hostnamed
.service-6QObBk
systemd-private-aba2802a36d142afafbc22d46fa798ab-mariadb.service-x
HRETT
systemd-private-b4b460c159024bd7833e0d2618116862-mariadb.service-b
Y14Ma
systemd-private-b4b460c159024bd7833e0d2618116862-nginx.service-WGP
tvZ
systemd-private-bdcb3e2b6c4e4c8e8db52cd69c9da50a-systemd-hostnamed
.service-c69XjR
systemd-private-c3dcda9db3f44d7b9778b552335da333-systemd-hostnamed
.service-uXlDLZ
systemd-private-c5d6faaf58ea43a2a178e8c32c6d4ef9-systemd-hostnamed
.service-jOTscz
systemd-private-c9d5241aa719451e87e5c135f61c02a9-mariadb.service-L
6sroc
systemd-private-c9d5241aa719451e87e5c135f61c02a9-nginx.service-gUl
hK9
systemd-private-d1cbada33dbe45b09c51747a8ba0df5c-mariadb.service-d
cZ3PU
systemd-private-d5a16560059e47a88fd3023b3aa6ad65-mariadb.service-7
oRvMa
systemd-private-d5a16560059e47a88fd3023b3aa6ad65-nginx.service-jE1
4Ru
systemd-private-e56069f99d7549eb9659feda00a5b4d0-mariadb.service-y
T4QjG
systemd-private-e56069f99d7549eb9659feda00a5b4d0-nginx.service-c8R
ojC
tutordeLpwJ
uploadingfiles.zip
 
3. Listar todos los archivos del directorio etc que empiecen por t en orden inverso.
 
[ferhandel@webminal.org ~]$ls /tmp
npm-26730-c0468a8c
[ferhandel@webminal.org ~]$ls -r /etc/t*
ls: cannot access /etc/tuned: Permission denied
/etc/tcsd.conf
 
/etc/tmpfiles.d:
 
/etc/terminfo:
 
4. Listar todos los archivos del directorio dev que empiecen por tty y tengan 5 caracteres.
 
[ferhandel@webminal.org ~]$ls /dev/tty??
ls: cannot access /dev/tty10: Permission denied
ls: cannot access /dev/tty11: Permission denied
ls: cannot access /dev/tty12: Permission denied
ls: cannot access /dev/tty13: Permission denied
ls: cannot access /dev/tty14: Permission denied
ls: cannot access /dev/tty15: Permission denied
ls: cannot access /dev/tty16: Permission denied
ls: cannot access /dev/tty17: Permission denied
ls: cannot access /dev/tty18: Permission denied
ls: cannot access /dev/tty19: Permission denied
ls: cannot access /dev/tty20: Permission denied
ls: cannot access /dev/tty21: Permission denied
ls: cannot access /dev/tty22: Permission denied
ls: cannot access /dev/tty23: Permission denied
ls: cannot access /dev/tty24: Permission denied
ls: cannot access /dev/tty25: Permission denied
ls: cannot access /dev/tty26: Permission denied
ls: cannot access /dev/tty27: Permission denied
ls: cannot access /dev/tty28: Permission denied
ls: cannot access /dev/tty29: Permission denied
ls: cannot access /dev/tty30: Permission denied
ls: cannot access /dev/tty31: Permission denied
ls: cannot access /dev/tty32: Permission denied
ls: cannot access /dev/tty33: Permission denied
ls: cannot access /dev/tty34: Permission denied
ls: cannot access /dev/tty35: Permission denied
ls: cannot access /dev/tty36: Permission denied
ls: cannot access /dev/tty37: Permission denied
ls: cannot access /dev/tty38: Permission denied
ls: cannot access /dev/tty39: Permission denied
ls: cannot access /dev/tty40: Permission denied
ls: cannot access /dev/tty41: Permission denied
ls: cannot access /dev/tty42: Permission denied
ls: cannot access /dev/tty43: Permission denied
ls: cannot access /dev/tty44: Permission denied
ls: cannot access /dev/tty45: Permission denied
ls: cannot access /dev/tty46: Permission denied
ls: cannot access /dev/tty47: Permission denied
ls: cannot access /dev/tty48: Permission denied
ls: cannot access /dev/tty49: Permission denied
ls: cannot access /dev/tty50: Permission denied
ls: cannot access /dev/tty51: Permission denied
ls: cannot access /dev/tty52: Permission denied
ls: cannot access /dev/tty53: Permission denied
ls: cannot access /dev/tty54: Permission denied
ls: cannot access /dev/tty55: Permission denied
ls: cannot access /dev/tty56: Permission denied
ls: cannot access /dev/tty57: Permission denied
ls: cannot access /dev/tty58: Permission denied
ls: cannot access /dev/tty59: Permission denied
ls: cannot access /dev/tty60: Permission denied
ls: cannot access /dev/tty61: Permission denied
ls: cannot access /dev/tty62: Permission denied
ls: cannot access /dev/tty63: Permission denied
ls: cannot access /dev/ttyS0: Permission denied
ls: cannot access /dev/ttyS1: Permission denied
ls: cannot access /dev/ttyS2: Permission denied
ls: cannot access /dev/ttyS3: Permission denied
 
5. Listar todos los archivos del directorio dev que empiecen por tty y acaben en 1,2,3 ó 4.
 
[ferhandel@webminal.org ~]$ls /dev/tty*[1-4]
ls: cannot access /dev/tty1: Permission denied
ls: cannot access /dev/tty11: Permission denied
ls: cannot access /dev/tty12: Permission denied
ls: cannot access /dev/tty13: Permission denied
ls: cannot access /dev/tty14: Permission denied
ls: cannot access /dev/tty2: Permission denied
ls: cannot access /dev/tty21: Permission denied
ls: cannot access /dev/tty22: Permission denied
ls: cannot access /dev/tty23: Permission denied
ls: cannot access /dev/tty24: Permission denied
ls: cannot access /dev/tty3: Permission denied
ls: cannot access /dev/tty31: Permission denied
ls: cannot access /dev/tty32: Permission denied
ls: cannot access /dev/tty33: Permission denied
ls: cannot access /dev/tty34: Permission denied
ls: cannot access /dev/tty4: Permission denied
ls: cannot access /dev/tty41: Permission denied
ls: cannot access /dev/tty42: Permission denied
ls: cannot access /dev/tty43: Permission denied
ls: cannot access /dev/tty44: Permission denied
ls: cannot access /dev/tty51: Permission denied
ls: cannot access /dev/tty52: Permission denied
ls: cannot access /dev/tty53: Permission denied
ls: cannot access /dev/tty54: Permission denied
ls: cannot access /dev/tty61: Permission denied
ls: cannot access /dev/tty62: Permission denied
ls: cannot access /dev/tty63: Permission denied
ls: cannot access /dev/ttyS1: Permission denied
ls: cannot access /dev/ttyS2: Permission denied
ls: cannot access /dev/ttyS3: Permission denied
 
6. Listar todos los archivos del directorio dev que empiecen por t y acaben en C1.
 
[ferhandel@webminal.org ~]$ls /dev/t*c1                          
ls: cannot access /dev/t*c1: No such file or directory
 
7. Listar todos los archivos, incluidos los ocultos, del directorio raíz.
 
[ferhandel@webminal.org ~]$ls -a /.
.     common_pool  lib         mnt   run   test         usr
..    dev          lib64       opt   sbin  test2        var
bin   etc          lost+found  proc  srv   tmp
boot  home         media       root  sys   tmpuserdata
 
8. Listar todos los archivos del directorio etc que no empiecen por t.
 
[ferhandel@webminal.org ~]$ls -d /etc/[^t]*                      
ls: cannot access /etc/adjtime: Permission denied
ls: cannot access /etc/aliases: Permission denied
ls: cannot access /etc/aliases.db: Permission denied
ls: cannot access /etc/audit: Permission denied
ls: cannot access /etc/cron.d: Permission denied
ls: cannot access /etc/crontab: Permission denied
ls: cannot access /etc/dbus-1: Permission denied
ls: cannot access /etc/dhcp: Permission denied
ls: cannot access /etc/dnsmasq.conf: Permission denied
ls: cannot access /etc/dnsmasq.d: Permission denied
ls: cannot access /etc/exports: Permission denied
ls: cannot access /etc/firewalld: Permission denied
ls: cannot access /etc/fstab: Permission denied
ls: cannot access /etc/gconf: Permission denied
ls: cannot access /etc/gshadow: Permission denied
ls: cannot access /etc/gshadow-: Permission denied
ls: cannot access /etc/hosts: Permission denied
ls: cannot access /etc/kdump.conf: Permission denied
ls: cannot access /etc/libvirt: Permission denied
ls: cannot access /etc/lvm: Permission denied
ls: cannot access /etc/modprobe.d: Permission denied
ls: cannot access /etc/mtab~: Permission denied
ls: cannot access /etc/mtaz~: Permission denied
ls: cannot access /etc/my.cnf: Permission denied
ls: cannot access /etc/my.cnf.d: Permission denied
ls: cannot access /etc/NetworkManager: Permission denied
ls: cannot access /etc/nginx: Permission denied
ls: cannot access /etc/postfix: Permission denied
ls: cannot access /etc/ppp: Permission denied
ls: cannot access /etc/radvd.conf: Permission denied
ls: cannot access /etc/redis-sentinel.conf: Permission denied
ls: cannot access /etc/rsyncd.conf: Permission denied
ls: cannot access /etc/rsyslog.conf: Permission denied
ls: cannot access /etc/rsyslog.d: Permission denied
ls: cannot access /etc/shadow: Permission denied
ls: cannot access /etc/shadow-: Permission denied
ls: cannot access /etc/shadow.bak: Permission denied
ls: cannot access /etc/shadow.bak2: Permission denied
ls: cannot access /etc/shadow_bak_may11: Permission denied
ls: cannot access /etc/shadow_jnu2: Permission denied
ls: cannot access /etc/shadow_may19: Permission denied
/etc/alternatives                     /etc/mtools.conf
/etc/anacrontab                       /etc/nanorc
/etc/asound.conf                      /etc/netconfig
/etc/audisp                           /etc/networks
/etc/avahi                            /etc/nfs.conf
/etc/bash_completion.d                /etc/nfsmount.conf
/etc/bashrc                           /etc/nsswitch.conf
/etc/binfmt.d                         /etc/nsswitch.conf.bak
/etc/centos-release                   /etc/nsswitch.conf.rpmnew
/etc/centos-release-upstream          /etc/numad.conf
/etc/chkconfig.d                      /etc/openldap
/etc/cloud                            /etc/opt
/etc/cron.daily                       /etc/os-release
/etc/cron.deny                        /etc/pam.d
/etc/cron.hourly                      /etc/passwd
/etc/cron.monthly                     /etc/passwd-
/etc/cron.weekly                      /etc/passwd.bak
/etc/crypttab                         /etc/pkcs11
/etc/csh.cshrc                        /etc/pki
/etc/csh.login                        /etc/plymouth
/etc/dconf                            /etc/pm
/etc/default                          /etc/polkit-1
/etc/depmod.d                         /etc/popt.d
                                                                 
/etc/DIR_COLORS                       /etc/prelink.conf.d
/etc/DIR_COLORS.256color              /etc/printcap              
/etc/dracut.conf                      /etc/profile.d
                                                                 
/etc/dracut.conf.d                    /etc/protocols
/etc/e2fsck.conf                      /etc/pulse
/etc/ethertypes                       /etc/qemu-kvm
                                                                 
/etc/exports.d                        /etc/rc0.d
/etc/extlinux.conf                    /etc/rc1.d
/etc/filesystems                      /etc/rc3.d
                                                                 
/etc/fonts                            /etc/rc4.d
/etc/fstabe                           /etc/rc5.d
/etc/gcrypt                           /etc/rc.dd
                                                                 
/etc/gdbinit                          /etc/rc.local
/etc/gdbinit.d                        /etc/rdma
/etc/GeoIP.conf.default               /etc/redis.confease
                                                                 
/etc/ghostscript                      /etc/request-key.conf
/etc/gnupg                            /etc/request-key.d
/etc/groff                            /etc/resolv.conf.GEEN9Z    
                                                                 
/etc/group                            /etc/rpc                    
/etc/group-                           /etc/rpm
/etc/group_all_find                   /etc/rwtab.d
                                                                 
/etc/group_all_fine                   /etc/sasl2
/etc/group.bak                        /etc/screenrc              
/etc/grub.d                           /etc/securityy
                                                                 
/etc/gss                              /etc/selinux
/etc/gssproxy                         /etc/services
/etc/hostname                         /etc/shellsus.conf          
                                                                 
/etc/hosts.allow                      /etc/skel
/etc/hosts.deny                       /etc/ssh
/etc/ImageMagick                      /etc/statetab
                                                                 
/etc/init.d                           /etc/statetab.d
/etc/inittab                          /etc/subgid-                
/etc/iproute2                         /etc/subuid-bak            
                                                                 
/etc/iscsi                            /etc/subuid.bak
/etc/issue                            /etc/sudo.conf              
/etc/kernel                           /etc/sudoers.d
                                                                 
/etc/krb5.conf                        /etc/sudoers.rpmnew        
/etc/krb5.conf.d                      /etc/sudo-ldap.conf
/etc/ld.so.cache                      /etc/sysctl.conf
                                                                 
/etc/ld.so.conf                       /etc/sysctl.d
/etc/ld.so.conf.d                     /etc/systemd
/etc/libaudit.conf                    /etc/system-release-cpe
                                                                 
/etc/libguestfs-tools.conf            /etc/udev
/etc/libibverbs.d                     /etc/uwsgi.ini
/etc/libreport                        /etc/vimrcole.conf
                                                                 
/etc/libuser.conf                     /etc/virc                  
/etc/locale.conf                      /etc/virt-builder
/etc/login.defs                       /etc/wmsudo
                                                                 
/etc/logrotate.conf                   /etc/wmsudo.txt            
/etc/logrotate.d                      /etc/wpa_supplicant
/etc/magic                            /etc/xdg
                                                                 
/etc/makedumpfile.conf.sample         /etc/xinetd.d
/etc/man_db.conf                      /etc/yum
/etc/mke2fs.conf                      /etc/yum.repos.d
                                                                 
/etc/modules-load.d                   /etc/zlogin
/etc/motd                             /etc/zlogout                
/etc/motd_jun2                        /etc/zshenvle              
                                                                 
/etc/mtab                             /etc/zshrc
 
9. Listar todos los archivos del directorio usr y sus subdirectorios.
 
ls -R /usr (esto es infinito asi que te dejo aqui el comando)
 
10. Cambiarse al directorio tmp.
 
[ferhandel@webminal.org ~]$cd /tmp                                
[ferhandel@webminal.org tmp]$

11. Verificar que el directorio actual ha cambiado.

[ferhandel@webminal.org tmp]$pwd
/tmp

12. Mostrar el día y la hora actual. 

[ferhandel@webminal.org tmp]$date   
Thu Nov 25 20:03:49 CET 2021 

13. Con un solo comando posicionarse en el directorio $HOME.

[ferhandel@webminal.org tmp]$cd /home

[ferhandel@webminal.org home]$

14. Verificar que se está en él.

[ferhandel@webminal.org home]$pwd   
/home                               

15. Listar todos los ficheros del directorio HOME mostrando su número de inodo.

ls -i386

16. Borrar todos los archivos y directorios visibles de vuestro directorio PRUEBA.

[ferhandel@webminal.org ~]$rm -rf PRUEBA/*

17. Crear los directorios dir1, dir2 y dir3 en el directorio PRUEBA. Dentro de dir1 crear el directorio dir11. 
Dentro del directorio dir3 crear el directorio dir31. 
Dentro del directorio dir31, crear los directorios dir311 y dir312. 


[ferhandel@webminal.org ~]$cd PRUEBA
[ferhandel@webminal.org PRUEBA]$
[ferhandel@webminal.org PRUEBA]$mkdir 
dir1
[ferhandel@webminal.org PRUEBA]$mkdir 
dir2
[ferhandel@webminal.org PRUEBA]$mkdir 
dir3
[ferhandel@webminal.org ~]$cd dir1/
[ferhandel@webminal.org dir1]$mkdir -v dir11/                                                                                        
mkdir: created directory ‘dir11/’                                                                                                    
[ferhandel@webminal.org dir1]$ls                                                                                                     
dir11
[ferhandel@webminal.org ~]$cd dir3/                                                                                                  
[ferhandel@webminal.org dir3]$mkdir -v dir31/                                                                                        
mkdir: created directory ‘dir31/’ 
[ferhandel@webminal.org ~]$cd dir31/
[ferhandel@webminal.org dir31]$mkdir -v dir311/                                                                                      
mkdir: created directory ‘dir311/’                                                                                                   
[ferhandel@webminal.org dir31]$mkdir -v dir312/                                                                                      
mkdir: created directory ‘dir312/’ 

18. Copiar el archivo /etc/motd a un archivo llamado mensaje de vuestro directorio PRUEBA.

[ferhandel@webminal.org dir31]$sudo touch /etc/motd/PRUEBA/mensaje

19. Copiar mensaje en dir1, dir2 y dir3. 

[ferhandel@webminal.org ~]$cd PRUEBA
[ferhandel@webminal.org PRUEBA]$cp mensaje dir1/mensaje
[ferhandel@webminal.org PRUEBA]$cp mensaje dir2/mensaje
[ferhandel@webminal.org PRUEBA]$cp mensaje dir3/mensaje

20. Comprobar el ejercicio anterior mediante un solo comando. 
(A partir de aquí no funciona webminal)

cd PRUEBA

ls -R PRUEBA

21. Copiar los archivos del directorio rc.d que se encuentra en /etc al directorio dir31.

cp -r /etc/rc.d dir31

22. Copiar en el directorio dir311 los archivos de /bin que tengan una a como segunda letra y su nombre tenga cuatro letras. 

cp -r /bin/?a?? PRUEBA/dir3/dir31/dir311

23. Copiar el directorio de otro usuario y sus subdirectorios debajo de dir11 (incluido el propio directorio).

sudo cp -r ../usuario2 PRUEBA/dir1/dir11

24. Mover el directorio dir31 y sus subdirectorios debajo de dir2. 

mv PRUEBA/dir3/dir31 PRUEBA/dir2

25. Mostrar por pantalla los archivos ordinarios del directorio HOME y sus subdirectorios. 

ls -R $HOME 

26. Ocultar el archivo mensaje del directorio dir3. 

mv PRUEBA/dir3/mensaje PRUEBA/dir3/.mensaje 

27. Borrar los archivos y directorios de dir1, incluido el propio directorio. 28. Copiar al directorio dir312 los ficheros del directorio /dev que empiecen por t, acaben en una letra que vaya de la a a la b y tengan cinco letras en su nombre. 

rm -rf PRUEBA/dir1

29. Borrar los archivos de dir312 que no acaben en b y tengan una q como cuarta letra.

rm -r PRUEBA/dir2/dir31/dir312/???q[^b]

30. Mover el directorio dir312 debajo de dir3. 

mv PRUEBA/dir2/dir31/dir312 PRUEBA/dir3

31. Crear un enlace simbólico al directorio dir1 dentro del directorio dir3 llamado enlacedir1.

ln -s /home/windows/PRUEBA/dir1 PRUEBA/dir3/enlacedir1

32. Posicionarse en dir3 y, empleando el enlace creado en el ejercicio anterior, crear el directorio nuevo1 dentro de dir1.

cd dir3 mkdir enlacedir1/nuevo1

33. Utilizando el enlace creado copiar los archivos que empiecen por u del directorio /bin en directorio nuevo1.

cp -r /bin/u* enlacedir1/nuevo1/

34. Crear dos enlaces duros del fichero fich1, llamarlo enlace, en los directorios dir1 y dir2. 

ln fich1 dir1/enlace ln fich1 dir2/enlace

35. Borrar el archivo fich1 y copiar enlace en dir3.

-rm fich1 cp dir1/enlace dir3/

36. Crear un enlace simbólico (llamado enlafich1) al fichero enlace de dir2 en dir1. 

ln -s /home/windows/PRUEBA/dir2
/home/windows/PRUEBA/dir1/enlafich1

37. Posicionarse en dir1 y, mediante el enlace creado, copiar el archivo fichl dentro de dir311. 

cd dir1 cp enlafich1 ../dir2/dir31/dir311/fich1

38. Seguir en dir1 y, mediante el enlace creado, sacar por pantalla las líneas que tiene el archivo fich1. 

cat fich1

39. Borrar el fichero fich1 de dir2 

rm dir2/fich1

40. Borrar todos los archivos y directorios creados durante los ejercicios. 

rm -r *

41. Crear el directorio dir2 y dir3 en el directorio PRUEBA ¿Cuáles son los actuales permisos del directorio dir2? 

mkdir dir1 mkdir dir2 ls -l

42. Utilizando la notación simbólica, eliminar todos los permisos de escritura (propietario, grupo, otros) del directorio dir2. 

chmod 555 dir2

43. Utilizando la notación octal, eliminar el permiso de lectura del directorio dir2, al resto de los usuarios. 

chmod 551 dir2

44. ¿Cuáles son ahora los permisos asociados a dir2? 

ls -l

45. Crear bajo dir2, un directorio llamado dir2l.

mkdir dir2/dir21

46. Concederse a sí mismo permiso de escritura en el directorio dir2 e intentar de nuevo el paso anterior. 

chmod 751 dir2 mkdir dir2/dir21

47. ¿Cuáles son los valores por omisión asignados a los archivos? 48. Cambiar el directorio actual al directorio dir3. Imprimir su trayectoria completa para verificar el cambio. 

ls -l dir2

49. ¿Cuáles son los permisos asignados en su momento a este directorio? 

ls -lR

50. Establecer mediante el comando umask (buscar este comando) los siguientes valores por omisión: rwxr--r-- para los directorios y rw-r--r-- para los archivos ordinarios. 



51. Crear cuatro nuevos directorios llamados dira, dirb, dirc, y dird bajo el directorio actual. 

mkdir dira mkdir dirb mkdir dirc mkdir dird

52. Comprobar los permisos de acceso de los directorios recién creados para comprobar el funcionamiento del comando umask. 

ls -l

53. Crear el fichero uno . Quitarle todos los permisos de lectura. Comprobarlo. Intentar borrar dicho fichero. 

touch uno chmod a-r uno ls -l rm uno

54. Quitarle todos los permisos de paso al directorio dir2 y otorgarle todos los demás.

chmod = dir2 chmod o=rwx dir2

55. Crear en el directorio propio: 
El directorio carpeta1 con los tres permisos para el propietario, dentro de él fich1 con lectura y escritura para todos y fich2 con lectura y escritura para el propietario y solo lectura para el resto. 
El directorio carpeta2 con todos los permisos para el propietario y lectura y ejecución para los del mismo grupo. 
Dentro file1 con lectura y escritura para el propietario y los del grupo y file2 con los mismos para el propietario y solo lectura para el grupo.

mkdir carpeta1 chmod u=rwx,g=,o= carpeta1 touch carpeta1/fich1
touch carpeta1/fich2 chmod = carpeta1/fich1 chmod = carpeta1/fich2 chmod o=rw carpeta1/fich1 ls -l
mkdir carpeta2 chmod u=rwx,g=rx,o= carpeta2 touch carpeta2/file1 touch carpeta2/file2 chmod = carpeta2/file2 chmod = carpeta2/file1 chmod u=rw,g=rw carpeta2/file1 chmod u=rw,g=r carpeta2/file2 ls -l

56. Desde otro usuario probar todas las operaciones que se pueden hacer en los ficheros y directorios creados.

ls -lR 

57. Visualizar la trayectoria completa del directorio actual. Crear dos directorios llamados correo y fuentes debajo del directorio actual.

mkdir correo mkdir fuentes

58. Posicionarse en el directorio fuentes y crear los directorios dir1, dir2, dir3. 

cd fuentes mkdir dir1 mkdir dir2 mkdir dir3

59. Crear el directorio menus bajo correo sin moverse del directorio actual. 

mkdir ../correo/menus

60. Posicionarse en el directorio HOME. Borrar los directorios que cuelgan de fuentes que acaben en un número que no sea el 1. 

cd $HOME
rm -r fuentes/ 

61. Ver si existe el archivo tty2 en el directorio dev. En caso de que exista, ver su fecha de creación o actualización. 

find PRUEBA/fuentes -type d -name "tty2" -exec ls -l {} ;

62. Ver los permisos que tienen los archivos que empiecen por tt del directorio /dev. 

ls -l /dev/tt*

63. Visualizar la lista de los archivos ordinarios que están en el directorio /usr/bin. 

find /usr/bin -type d -name "*" -exec ls -l {} ; 

64. Visualizar la lista de todos los directorios que cuelgan del raíz. 

find / -type d -name "*" -exec ls {} ;

65. Visualizar la lista de todos los ficheros que pertenezcan a root. 

find / -user root -type f

66. Visualizar la lista de todos los ficheros .h del directorio /usr/include. 

find /usr/include -type f -regex ".*.h"

67. Ejecutar todos los comandos que empiecen por ls del directorio /bin.

ls /bin/ls*

68. Visualizar de qué tipo son todos y cada uno de ficheros de todo el árbol del sistema propiedad de un usuario conocido. 

find /home/ubuntu -exec file --mime-type -0 '{}' ;

69. Crear el directorio uno en el directorio HOME con permiso de escritura y paso para el propietario, de lectura y paso para los usuarios de su mismo grupo y ningún permiso para el resto de usuarios.

mkdir uno chmod u=rw,g=rw,o= uno ls -ld uno

70. Crear el directorio uno1 dentro del directorio creado en el ejercicio anterior con todos lo permisos para el usuario, ninguno para los usuarios del grupo y permiso de escritura para el resto de usuarios.

chmod u=rwx,g=rwx,o= uno mkdir uno/uno1 chmod 
u=rwx,g=,o=w uno/uno1 ls -ld uno/uno1

71. Copiar todos los ficheros propiedad de un usuario conocido que acaben en un número en el directorio menus. 

find /home/usuario2 -type f -regex ".*[0-9]" -exec cp -r '{}' PRUEBA/correo/menus/ ;

72. Visualiza con la orden who la relación de usuarios conectados y sus terminales. Mediante la orden cat, crea un pequeño mensaje desde tu consola y redirígelo a uno de los terminales conectados.



73. Crea un archivo de tamaño 0 

touch archivo

74. Visualiza el archivo /etc/motd, que contiene el "mensaje del día". 

cat /etc/motd

75. Utilizando de entrada la información de los usuarios conectados al sistema, guardar, ordenadas por el campo hora, las líneas correspondientes al usuario que se desee en el archivo persona. 

who | grep $USER | sort -k 4 > persona

76. Crear el directorio carpeta debajo del directorio PRUEBA. Quitarle todos los permisos de lectura. A continuación, buscar todos los directorios que cuelguen del directorio propio y guardarlos en el archivo direc. 

mkdir -p carpeta/PRUEBA 
chmod a-r carpeta 
find ~ -type d > direc

77. Volver a realizar la segunda parte del ejercicio anterior, pero redireccionando los errores al fichero malos. Comprobar la información del fichero malos

find ~ -type d 2> malo

78. Añadir al fichero direc la lista de todos los ficheros ordinarios que cuelguen de /etc. 

find /etc -type f >> direc

79. Añadir al archivo nuevalista el/los nombre/s de el/los fichero/s del directorio PRUEBA que contengan en su nombre la cadena "ai", añadiendo el posible error al fichero malos. 



80. Sacar por pantalla únicamente el tiempo (buscar comando time) que tarda en ejecutarse el comando who.

time who 

81. Sacar por pantalla un listado completo (buscar comando ps) de los procesos que está realizando el usuario root. 

ps

82. Crear el archivo proceso con los procesos que no tienen ningún terminal asignado.

touch archivo
(incompleto)

83. Añadir al fichero anterior la fecha actual y la trayectoria completa del directorio actual.

echo "'date +"%A %D"' - 'pwd'" >>nuevalista

84. Sacar por pantalla el listado de todos los usuarios conectados ordenados por número de proceso asignado. 

ps axu

85. Averiguar cuál es la actividad actual del sistema. Para ello visualice un listado completo del estado de todos los procesos que se están ejecutando en el sistema. 

top

86. Obtener un listado con los siguientes datos de los procesos de su shell actual.

ps -e

87. Mostrar cuantos usuarios tiene registrados el sistema (el registro de usuarios está en el archivo /etc/passwd) 

cat /etc/passwd | wc -l

88. Mostrar cuántos usuarios tiene registrados el sistema y que utilizan el intérprete bash (debe aparecer al final de la línea /bin/bash o similar) 

cat /etc/passwd | grep /bash

89. Mostrar cuantos usuarios hay conectados 

who -q

90. Mostrar las líneas, de un archivo de texto, empiecen por L (mayúscula o minúscula) 



91. Contar las líneas, del ejemplo anterior 




























